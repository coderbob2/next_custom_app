	...
	next_custom_app.procurement_workflow = {
		...
		add_create_buttons: function (frm) {
			...
			function user_has_role(role) {
				...
				var user_roles = frappe.user_roles || (frappe.boot && frappe.boot.user && frappe.boot.user.roles) || [];
				return user_roles.indexOf(role) !== -1 || user_roles.indexOf('Administrator') !== -1;
			}

			// Get the next steps in workflow (with role info)
			frappe.call({
					...
					if (r.message && Array.isArray(r.message) && r.message.length) {
						...
						r.message.forEach(function (step) {
							...
							if (!user_has_role(step.role)) {
								console.log("Skipping button for", step.doctype_name, "- user lacks role:", step.role);
								return;
							}

							var next_doctype = step.doctype_name;
							var next_doctype_label = next_doctype;
							if (next_doctype === 'Stock Entry') {
								var purpose = frm.doc && (frm.doc.material_request_type || frm.doc.purchase_requisition_type || frm.doc.purpose)
				...
				error: function (r) {
					console.error("Error getting next steps:", r);
				}
			});
		},
